...
/// OPPO Software Store (OPPO 软件商店) IAP via OPPO Pay SDK.
///
/// Native side (Kotlin) to implement:
///   MethodChannel "com.nightglow/oppo_iap"
///   Methods: isAvailable, loadProducts, buyProduct, restorePurchases
///   EventChannel "com.nightglow/oppo_iap_events" → pushes purchase events
///
/// Required setup:
///   1. Register on OPPO Open Platform (open.oppomobile.com), create app & products
///   2. Download OPPO Pay SDK and add AAR to android/app/libs/
///   3. Add dependency in android/app/build.gradle.kts:
///        implementation(files("libs/ocloud-saas-android-pay.aar"))
///   4. Implement MethodChannel handler in MainActivity.kt
class OppoPurchaseRepository implements PurchaseRepository {
  static const _method = MethodChannel('com.nightglow/oppo_iap');
  static const _events = EventChannel('com.nightglow/oppo_iap_events');

  final StreamController<List<StorePurchaseUpdate>> _controller =
      StreamController.broadcast();

  OppoPurchaseRepository() {
  ...
  String get subscriptionTermsText =>
      '订阅将在到期前自动续费并从 OPPO 账号余额/绑定支付方式扣款。'
      '可在 OPPO 软件商店 → 我的 → 订阅 中管理或取消。';
